...
✅ Maintenance Priority Matrix (remplace Cost Analysis)
✅ Maintenance Gap Index
✅ Infrastructure Degradation Rate
✅ Top 10 Schools Urgent
✅ Compare Provinces
✅ Schools at Risk Score
...
def calculate_degradation_rate(data):
    ...
    if avg_days > 0:
        ...
        return round(min(degradation_per_year, 20), 2)
    return 0

def calculate_urgency_score(row):
    """Calculer score d'urgence (0-10)"""
    score = 0
    score += row['m5_delayed_maintenance'] * 4
    score += (row['m2_days_since_last_maintenance'] > 365) * 3
    score += (row['m2_days_since_last_maintenance'] > 180) * 2
    score += (row['m6_funding_source_diversity'] < 0.3) * 1
    return min(score, 10)

def calculate_importance_score(row):
    """Calculer score d'importance (0-10)"""
    score = 0
    score += (1 - row['index_1_infrastructure_health_index']) * 5
    score += (row['number_of_students'] / 1000) * 3
    score += (row['m3_capitation_grant_pct'] < 15) * 2
    return min(score, 10)
...
app.layout = dbc.Container([
    ...
    dbc.Row([
        ...
        dbc.Col([
            dbc.Card([
                dbc.CardHeader("🎯 MAINTENANCE PRIORITY MATRIX", 
                              style={'fontWeight': 'bold', 'backgroundColor': '#d4edda', 'fontSize': '12px', 'padding': '6px'}),
                dbc.CardBody([
                    dcc.Graph(id='priority-matrix', style={'height': '320px'}, config={'displayModeBar': False})
                ], style={'padding': '5px'})
...
def update_priority_matrix(province, district, sector):
    filtered_df = filter_data(
        province=province if province != 'All Provinces' else None,
        district=district if district != 'All Districts' else None,
        sector=sector if sector != 'All Sectors' else None
    ...
    matrix_df['urgency'] = matrix_df.apply(calculate_urgency_score, axis=1)
    matrix_df['importance'] = matrix_df.apply(calculate_importance_score, axis=1)
    
    # Quadrant colors
    def get_quadrant_color(urg, imp):
        if urg >= 5 and imp >= 5:
            return '#d62728'  # DO NOW - Red
        elif urg < 5 and imp >= 5:
            return '#ffa500'  # PLAN - Orange
        elif urg >= 5 and imp < 5:
            return '#ffcc00'  # DELEGATE - Yellow
        else:
            return '#2ca02c'  # LATER - Green
    
    matrix_df['color'] = matrix_df.apply(lambda row: get_quadrant_color(row['urgency'], row['importance']), axis=1)
    matrix_df['size'] = matrix_df['number_of_students'] / 10
    
    fig = go.Figure()
    
    # Add scatter points
    fig.add_trace(go.Scatter(
        x=matrix_df['importance'],
        y=matrix_df['urgency'],
        mode='markers',
        marker=dict(
            size=matrix_df['size'],
            color=matrix_df['color'],
            line=dict(width=1, color='white')
        ),
        text=matrix_df['school_name'],
        hovertemplate='<b>%{text}</b><br>Urgency: %{y:.1f}<br>Importance: %{x:.1f}<extra></extra>'
    ))
    
    # Add quadrant lines
    fig.add_shape(type="line", x0=5, x1=5, y0=0, y1=10, line=dict(color="gray", width=2, dash="dash"))
    fig.add_shape(type="line", x0=0, x1=10, y0=5, y1=5, line=dict(color="gray", width=2, dash="dash"))
    
    # Add quadrant labels
    fig.add_annotation(x=7.5, y=7.5, text="DO NOW<br>🔴", showarrow=False, font=dict(size=11, color='#d62728', family='Arial Black'))
    fig.add_annotation(x=2.5, y=7.5, text="DELEGATE<br>🟡", showarrow=False, font=dict(size=11, color='#ffcc00', family='Arial Black'))
    fig.add_annotation(x=7.5, y=2.5, text="PLAN<br>🟠", showarrow=False, font=dict(size=11, color='#ffa500', family='Arial Black'))
    fig.add_annotation(x=2.5, y=2.5, text="LATER<br>🟢", showarrow=False, font=dict(size=11, color='#2ca02c', family='Arial Black'))
    ...
    fig.update_layout(
        ...
        yaxis=dict(gridcolor='#e0e0e0', range=[0, 10])
    )
...
if __name__ == '__main__':
    ...
    print("   2. ✅ Maintenance Priority Matrix (replaces Cost Analysis)")
